...
public class NodeApp {
    ...
    static class Args {
        ...
        static Args parse(String[] a) {
            ...
            if (map.containsKey("--host")) {
                args.host = map.get("--host");
            }
            if (map.containsKey("--port")) {
                args.port = Integer.parseInt(map.get("--port"));
            }
            if (map.containsKey("--pass")) {
                args.pass = map.get("--pass");
            }
            if (map.containsKey("--name")) {
                args.name = map.get("--name");
            }
            return args;
        }
    }
    ...
    static String HB_KEY(long pid) {
        return "hb:" + pid;
    } // TTL heartbeat

    static String INFO_KEY(long pid) {
        return "node:info:" + pid;
    } // HSET name, startedAt, lastSeen เป็นตัวเก็บข้อมูล process
    ...
    static class Subscriber implements Runnable {
        ...
        Subscriber(Args a, State s) {
            this.args = a;
            this.st = s;
        } // เอาตัวที่รับมาเก็บไว้ในclass

        @Override
        public void run() {
            while (!st.shuttingDown && !Thread.currentThread().isInterrupted()) {// thread ปัจจุบันถูก interrupt
                                                                                 // แล้วหรือยัง ถ้าไม่ถูก interrupt
                                                                                 // มีค่า flase
                ...
                try (jedis) {
                    JedisPubSub jps = new JedisPubSub() {// สร้างตัวจัดการข้อความ
                        @Override
                        public void onMessage(String ch, String msg) {// รับข้อความจากช่อง CH_BROADCAST, CH_CONTROL,
                                                                      // CH_PRESENCE
                            if (CH_BROADCAST.equals(ch)) {// ถ้าเป็นข้อความจากช่อง CH_BROADCAST
                                ...
                                if (msg.startsWith("control:kill")) {
                                    long target = Long.parseLong(msg.split("\\s+")[1]);// msg[1]==pid PUBLISH control
                                                                                       // "control:kill 22556"
                    ...
                    jedis.subscribe(jps, CH_BROADCAST, CH_CONTROL, CH_PRESENCE); // ฟังช่อง CH_BROADCAST, CH_CONTROL,
                                                                                 // CH_PRESENCE
                } catch (Exception e) {
    ...
    static class Commander implements Runnable {
        ...
        @Override
        public void run() {
            BufferedReader br = new BufferedReader(new InputStreamReader(System.in)); // สร้าง object BufferedReader
                                                                                      // ที่ใช้สำหรับ
                                                                                      // อ่านข้อความจากคีย์บอร์ด (stdin)
                                                                                      // แบบทีละบรรทัด
            // new InputStreamReader(System.in) สร้าง object InputStreamReader ที่ใช้สำหรับ
            // อ่านข้อความจากคีย์บอร์ด (stdin) แบบทีละบรรทัด
            ...
            while (!st.shuttingDown && !Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    if (line.toLowerCase().startsWith("kill ")) {
                        ...
                        System.out.printf("[%-10s|CMD ] unknown: %s (use: 'kill random' | 'kill <pid>')%n", st.name,
                                line);
                    }
                } catch (Exception e) {
        ...
        public void run() {
            ...
            try (Jedis j = newJedis(a.host, a.port, a.pass)) {
                ...
                j.hset(INFO_KEY(st.pid), Map.of(
                        ...
                        "lastSeen", Long.toString(now)));// จัดข้อมูลเป็น record คล้าย ๆ กับ row ในตาราง database
            }
    ...
    static void publishPresenceWithStatus(Jedis j, long leaderPid) {
        long now = System.currentTimeMillis();
        List<String> members = new ArrayList<>(j.zrevrange(ZSET_MEMBERS, 0, -1));// ดึงข้อมูลจาก ZSET_MEMBERS database
                                                                                 // เริ่ม 0 ถึง สุดท้าย
    ...
    public static void main(String[] argsArr) {
        ...
        if (pid < 0)
            pid = new Random().nextInt(1_000_000);
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            ...
            try (Jedis j = newJedis(args.host, args.port, args.pass)) {
                ...
                if (st.isLeader)
                    j.publish(CH_CONTROL, "control:leader -1");
            } catch (Exception ignore) {
            }
